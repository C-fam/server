...
# スプレッドシート名（例："keone_list_log"）は環境変数から取得
SPREADSHEET_NAME = os.getenv("SPREADSHEET_NAME", "keone_list_log")
# 有効なUIDのリストをカンマ区切りで指定（例： "12345,67890,23456" ）
VALID_UIDS = os.getenv("VALID_UIDS", "")
valid_uids = set(uid.strip() for uid in VALID_UIDS.split(",") if uid.strip())

# ----------------------------------
# Google Sheets 認証情報
# ----------------------------------
# credentials.json は使用せず、環境変数内にJSON文字列として保存します。
...
# 使用する各シート（それぞれ「Log」「guild_config」「granted_history」を用意してください）
LOG_SHEET = SPREADSHEET.worksheet("Log")
# guild_config, granted_history は動的に読み書きします

# ----------------------------------
# グローバル変数：各シートから読み込んだデータ
# ----------------------------------
# guild_config: {guild_id: {"guild_name": str, "channel_id": int, "role_id": int, "message_id": int, "debug_channel_id": int}}
guild_config = {}
# granted_history: {guild_id: [ {"uid": str, "username": str, "time": str}, ... ]}
granted_history = {}

# ----------------------------------
# guild_config シートの読み書き
# ----------------------------------
def load_guild_config():
    global guild_config
    guild_config = {}
    try:
        worksheet = SPREADSHEET.worksheet("guild_config")
        ...
        for row in rows:
            ...
            if gid:
                guild_config[gid] = {
                    ...
                    "debug_channel_id": int(row.get("debug_channel_id", 0)) if row.get("debug_channel_id") else 0
                }
    except Exception as e:
        print("Failed to load guild_config: ", e)

def save_guild_config():
    try:
        worksheet = SPREADSHEET.worksheet("guild_config")
        # シート全体をクリアし、ヘッダー行をセット
        worksheet.clear()
        header = ["guild_id", "guild_name", "channel_id", "role_id", "message_id", "debug_channel_id"]
        ...
        for gid, data in guild_config.items():
            worksheet.append_row([
                ...
                data.get("role_id", ""),
                data.get("message_id", ""),
                data.get("debug_channel_id", "")
            ])
    except Exception as e:
        print("Failed to save guild_config: ", e)

# ----------------------------------
# granted_history シートの読み書き
# ----------------------------------
def load_granted_history():
    global granted_history
    granted_history = {}
    try:
        worksheet = SPREADSHEET.worksheet("granted_history")
        rows = worksheet.get_all_records()
        for row in rows:
            gid = str(row.get("guild_id", ""))
            if gid:
        ...
        print("Failed to load granted_history: ", e)

def append_granted_history(gid, uid, username, timestamp):
    try:
        worksheet = SPREADSHEET.worksheet("granted_history")
        worksheet.append_row([gid, uid, username, timestamp])
    except Exception as e:
        print("Failed to append to granted_history: ", e)

# ----------------------------------
# Logシートへメッセージを追記する関数（必要に応じて利用）
# ----------------------------------
def append_log_to_sheet(message):
    try:
        timestamp = datetime.utcnow().isoformat()
        LOG_SHEET.append_row([timestamp, message])
    except Exception as e:
        print("Failed to append to Log sheet: ", e)
...
# ----------------------------------
# 永続的な UI コンポーネント: Check Eligibility ボタンとビュー
# ----------------------------------
class CheckEligibilityButton(discord.ui.Button):
    def __init__(self):
        super().__init__(
            custom_id="check_eligibility_button",  # 永続用の custom_id
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        append_granted_history(guild_id_str, user_id_str, str(interaction.user), datetime.utcnow().isoformat())
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    ...
    if guild:
        ...
        dbg_id = config.get("debug_channel_id", 0)
        if dbg_id:
...
async def extractinfo_command(interaction: discord.Interaction):
    ...
    lines.append(f"- Guild Name: {config.get('guild_name', '')}")
    lines.append(f"- Channel ID: {config.get('channel_id', '')}")
    lines.append(f"- Role ID: {config.get('role_id', '')}")
    lines.append(f"- Setup Message ID: {config.get('message_id', '')}")
    lines.append(f"- Debug Channel ID: {config.get('debug_channel_id', '')}")
    records = granted_history.get(guild_id_str, [])
    lines.append(f"\n**Recent Role Grants** (total {len(records)})")
